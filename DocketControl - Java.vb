        ...
        With rst
            ...
            Do Until .EOF
                Using Email As New MailMessage
                    strScratch = "Docket Control Item Due " & CStr(.Fields("DueDate").Value) & " (ID: " & CStr(.Fields("DocketID").Value) & ")"
                    ...
                    ElseIf .Fields("MarkID").Value > 0 Then
                        ...
                        If Not rstMark.EOF Then
                            If Len(rstMark.Fields("ResponsibleAtty").Value) > 0 Then
                                strScratch = strScratch & " ResponsibleAtty: " & rstMark.Fields("ResponsibleAtty").Value
                            End If
                        End If
                        rstMark.Close()
                    End If
                    Email.Subject = strScratch
                    With Email
...
HaveSubject:
                    ...
                    With rstNotify
                        ...
                        Else
                            ' strHTML = "<HTML><BODY>"
                            strHTML = "<HTML><HEAD><script type=""text/javascript""> " & _
                                                 "function runApp(which) { " & _
                                                 "WshShell = new ActiveXObject (""WScript.Shell""); " & _
                                                 "WshShell.Run(strCommand, [intWindowStyle], [bWaitOnReturn])" & _
                                                 "}  </script></HEAD><BODY>"
                            strTo1 = .Fields("Email").Value & "@evanspetree.com"
                            If Me.chkSMTPtest.CheckState = CheckState.Checked Then
                                Email.To.Add(New MailAddress(strGordonPrince))
                    ...
                    If bMark = True Then
                        ...
                        strScratch = strScratch & "<P>" & strCourierOn & "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Mark ID: </font>" & CStr(rstMark.Fields("MarkID").Value) & _
                                            " <a href=""WshShell.run(//localhost/C:/Tekhelps/ShowMark.bat%20" & CStr(rstMark.Fields("MarkID").Value) & """>Click to open Mark in IP Dashboard.</a>"
